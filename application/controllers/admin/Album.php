	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $gambar=$gbr['file_name'];
													...
													$this->m_album->simpan_album($album,$user_id,$user_nama,$gambar,$slug);
	...
	function update_album(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							unlink($path);
							$string   = preg_replace('/[^a-zA-Z0-9 \&%|{.}=,?!*()"-_+$@;<>\']/', '', $album_nama);
							$trim     = trim($string);
							$slug     = strtolower(str_replace(" ", "-", $trim));
							$kode=$this->session->userdata('idadmin');
							$user=$this->m_pengguna->get_pengguna_login($kode);
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_album->update_album($album_id,$album_nama,$user_id,$user_nama,$gambar,$slug);
	                    ...
	                    redirect('admin/album');
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_album->update_album_tanpa_img($album_id,$album_nama,$user_id,$user_nama,$slug);
							echo $this->session->set_flashdata('msg','info');
							redirect('admin/album');
	            } 
