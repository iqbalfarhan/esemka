...
class Album extends CI_Controller{
	...
	function index(){
        ...
        $config['base_url'] = base_url() . 'album/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						$x['data']=$this->m_album->berita_perpagee($offset,$limit);
						$x['category']=$this->db->get('tbl_album');
						$x['populer']=$this->db->query("SELECT * FROM tbl_album ORDER BY album_views DESC LIMIT 5");
						$this->load->template('depan/v_album',$x);
	}

	function detail($slugs){
		$slug=htmlspecialchars($slugs,ENT_QUOTES);
		$query = $this->db->get_where('tbl_album', array('album_slug' => $slug));
		if($query->num_rows() > 0){
			$b=$query->row_array();
			$kode=$b['album_id'];
			$this->db->query("UPDATE tbl_album SET album_views=album_views+1 WHERE album_id='$kode'");
			...
			$this->load->template('depan/v_jurusan_detail',$x);
		}else{
			redirect('kegiatan');
		}
	}
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_album.*,DATE_FORMAT(album_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_album WHERE album_nama LIKE '%$kategori%' ORDER BY album_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->get('tbl_album');
 			 $x['populer']=$this->db->query("SELECT * FROM tbl_album ORDER BY album_views DESC LIMIT 5");
			 $this->load->template('depan/v_jurusan',$x);
		 }else{
    ...
    function search(){
        ...
        $query=$this->m_album->cari_album($keyword);
				if($query->num_rows() > 0){
					$x['data']=$query;
					$x['category']=$this->db->get('tbl_album');
  				$x['populer']=$this->db->query("SELECT * FROM tbl_album ORDER BY album_views DESC LIMIT 5");
